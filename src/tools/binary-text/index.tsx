...
import { textToBinary, binaryToText } from '@core/index'
...
export default function BinaryText() {
  ...
  const runTransform = useCallback(() => {
    ...
    const result = mode === 'encode' ? textToBinary(input, separator) : binaryToText(input)
        ...
        {mode === 'encode' && (
          <div className="flex items-center gap-1 ml-2">
            ...
            {[
              ...
              <button
                key={s.label}
                onClick={() => setSeparator(s.value as Separator)}
                className={`px-2.5 py-1 rounded-md text-xs transition-colors
                  ${separator === s.value ? 'bg-accent text-bg-base' : 'bg-bg-raised text-text-muted hover:text-text-primary'}`}
              >
                {s.label}
              </button>
            ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 h-[calc(100vh-16rem)]">
        <div className="flex flex-col gap-2">
          ...
          <textarea
            ...
            placeholder={mode === 'encode' ? '输入文本...' : '输入二进制字符串 (0和1)...'}
            spellCheck={false}
          />
